export default function Body() {
  return (
    <>
      <div className="relative bg-gray-100 isolate overflow-hidden py-24 sm:py-32 min-h-screen text-green-800">
        <h1 className="text-center flex justify-center items-center flex-col opacity-100 text-7xl">
          <span className="italic">who&apos;s</span>
          <div className="font-extrabold">
            <span className="animate-bounce w-6 h-6">a</span>
            <span className="animate-bounce w-6 h-6">i</span>
            <span className="animate-bounce w-6 h-6">d</span>
            <span className="animate-bounce w-6 h-6">a</span>
            <span className="animate-bounce w-6 h-6">n</span>
            <span className="animate-bounce w-6 h-6"> </span>
            <span className="animate-bounce w-6 h-6">?</span>
          </div>
        </h1>
        <div className="text-center py-10">
          making cooasdfasdfasdfasdasldfamskldjf
        </div>
      </div>
    </>
  );
}
